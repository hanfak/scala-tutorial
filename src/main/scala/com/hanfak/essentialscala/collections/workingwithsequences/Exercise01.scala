package com.hanfak.essentialscala.collections.workingwithsequences
...
object Exercise01 extends App {
  ...
  println(s"${nolan.films.map(_.name)}")
  println(s"${directors.flatMap(_.films.map(_.name))}")
  println(s"${directors.map(_.films.map(_.name))}")
  println(s"${mcTiernan.films.sortWith { (film1, film2) =>
    film1.yearOfRelease < film2.yearOfRelease
  }.headOption.get.name}")
